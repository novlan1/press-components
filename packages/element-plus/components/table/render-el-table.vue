...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const filterChildrenVNode = (childrenVNode, checkedColumns) => childrenVNode.filter((_) => {
      const { label } = _.props;
      return checkedColumns.includes(label);
    });

    const getCheckboxGroupList = childrenVNode => childrenVNode.map((_) => {
      const { prop, label } = _.props;
      return { prop, label };
    });

    watch(() => props.checkedColumns, () => {
